...
# Находит две точки на прямой между r_atom и r_c на расстоянии distance от r_atom и выбирает ту, что ближе к r_c
def get_linear_place(r_atom: np.ndarray,
                     r_c: np.ndarray,
                     distance: float) -> np.ndarray:
    d = r_atom - r_c
    b = np.cross(np.cross(-r_atom, d), d) / np.dot(d, d)
    t = (np.dot(r_atom - b, d) + np.sqrt((np.dot(r_atom - b, d)) ** 2 - np.linalg.norm(d) ** 2 * (
            np.linalg.norm(r_atom) ** 2 + np.linalg.norm(b) ** 2 - 2 * np.dot(r_atom, b) - distance ** 2))) / (
                np.linalg.norm(d) ** 2)
    t_ = (np.dot(r_atom - b, d) - np.sqrt((np.dot(r_atom - b, d)) ** 2 - np.linalg.norm(d) ** 2 * (
            np.linalg.norm(r_atom) ** 2 + np.linalg.norm(b) ** 2 - 2 * np.dot(r_atom, b) - distance ** 2))) / (
                 np.linalg.norm(d) ** 2)
    r = b + t * d
    r_ = b + t_ * d
    if np.linalg.norm(r - r_c) > np.linalg.norm(r_ - r_c):
        return r_
    else:
        return r


# находит соседей каждого атома, используя словарь cutoff_dictionary
...
# Читает структуры в разных форматах
...
# Нужна, чтобы определить, есть ли вектор в списке
